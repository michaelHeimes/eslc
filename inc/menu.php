...
function trailhead_top_nav() {
	wp_nav_menu(array(
		...
		'items_wrap'		=> '<ul id="%1$s" class="%2$s" data-responsive-menu="accordion tablet-dropdown" data-submenu-toggle="true" data-disable-hover="true" data-click-open="true">%3$s</ul>',
	...
	if ( $args->theme_location == 'social-links') {
			...
			foreach( $items as &$item ) {
				...
				if( $icon ) {
					...
					$item->title = '<span class="icon-wrap grid-x align-middle align-center h-100" aria-hidden="true"><img class="style-svg" src="' . $icon['url'] . '" alt="' . $icon['alt'] . '"></span><span class="show-for-sr">' . $item->title . '</span>';
